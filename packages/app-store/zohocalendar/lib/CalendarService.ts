    ...
    return (
      data.freebusy
        ...
        .map((freebusy: FreeBusy) => ({
          ...
          start: dayjs.utc(freebusy.startTime?.replace('T', '')?.replace('Z', ''), "YYYYMMDDHHmmss").toISOString(),
          end: dayjs.utc(freebusy.endTime?.replace('T', '')?.replace('Z', ''), "YYYYMMDDHHmmss").toISOString(),
        })) || []
    );
  }
